...
Prepared requests have fixed provider endpoints, bounded JSON bodies, public headers, and a separately held secret header value. They are non-serializable and redact bodies, prompts, request IDs, responses, and authorization material from `Debug` and errors. OpenAI credentials are delivered only as an `Authorization: Bearer` header; Anthropic credentials are delivered only as `x-api-key`, with the explicit API-version header. The direct Rustls transport is constructed only behind an explicit paid runtime or ignored smoke-test gate; it cannot choose a different endpoint and resolves the protected secret only after request, budget, and reservation checks pass.

The only API implementation tool is `submit_patch`. It carries one bounded UTF-8 git diff to deterministic control-plane validation and has no provider-side shell or filesystem implementation. Risk class, capability, budget tool allowlist, exact scope, dual runtime acknowledgement, isolated-worktree application, and detached verification are required independently.
