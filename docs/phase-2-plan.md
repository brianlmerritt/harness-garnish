...
- Hardened rootless-Podman and Docker backends now have fail-closed runtime/effective-state attestation and explicit opt-in create/inspect/run/cleanup tests. The existing VPS/WSL2 runs establish capability selection; the VPS must run both real tests with locally available digest-pinned images to add backend conformance evidence.
- Real-agent claim execution, native desktop notification delivery, encrypted portable export, Apple Container conformance, and the remaining incomplete Phase 2 acceptance cases remain pending. In particular, P2-04, P2-05, and P2-07 still need their complete exclusion/ceiling/crash matrices before Phase 2 exit.
