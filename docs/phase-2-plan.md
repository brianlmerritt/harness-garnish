...
- A hardened rootless-Podman backend is implemented with fail-closed runtime and effective-state attestation. Its real create/inspect/run/cleanup test is explicitly opt-in through a digest-pinned image already present on the host; the existing VPS/WSL2 runs establish capability selection but must be rerun with `GARNISH_REAL_PODMAN_IMAGE` to add full Podman conformance evidence.
- Real-agent claim execution, native desktop notification delivery, encrypted portable export, Docker/Apple Container conformance, and the remaining incomplete Phase 2 acceptance cases remain pending. In particular, P2-04, P2-05, and P2-07 still need their complete exclusion/ceiling/crash matrices before Phase 2 exit.
