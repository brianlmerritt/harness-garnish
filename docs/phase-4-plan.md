...
The 2026-07-20 macOS quota-free suite for Schema 18 passed 137 tests with the 2 explicitly opt-in real-container tests ignored; strict Clippy also passed. Migration-backup, stable CLI JSON, stale/missing-plan rejection, atomic scheduler binding, and three-attempt worst-case reservation evidence are included. The portability script correctly refuses macOS, so native-Linux and WSL2 evidence remains required before this slice has cross-platform acceptance.

The 2026-07-20 macOS, native-Linux, and WSL2 quota-free suites each passed 135 tests with 2 opt-in real-container tests ignored. Format and strict Clippy passed on all three platforms. Each suite removed provider credential variables, used fake provider transports, and consumed no subscription/API quota. Exact kernels, toolchains, and the `5e635d2` evidence boundary are recorded in [`phase-4-portability-checkpoint.md`](phase-4-portability-checkpoint.md).
