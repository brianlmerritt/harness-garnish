...
Schema 17 adds that atomic scheduler boundary without enabling network transport. The exact-claim operation requires a matching paid-API task pin, validates and hashes the bounded request without persisting its prompt, derives worst-case currency from the effective price evidence, and commits the scheduler claim, capacity locks, and API reservation in one immediate transaction. A failed budget check rolls the entire claim back. Heartbeat and claim-to-run conversion keep the reservation aligned; scheduler stop, claim expiry, emergency stop, and pre-dispatch run cleanup release it once, while a dispatched reservation remains retained for settlement. Manual release and pre-run dispatch cannot bypass the binding. Exact claims currently require a zero-retry budget until multi-attempt reservation semantics are implemented. The general daemon still reports `api.scheduler_execution_unavailable` because durable per-task exact request plans are not yet an operator input. No credential or provider transport is used by this slice.
