...
Operator documentation now requires an explicit `Placeholders:` declaration before every copyable console block. A quota-free repository check enforces the declaration, and both CI and the Linux/WSL portability script run it. The portability environment scrub includes every API and Codex live selector, acknowledgement, receipt override, and real-container selector. The final macOS suite still passes 159 tests with 5 external tests ignored; fresh native-Linux and WSL2 runs remain required.
